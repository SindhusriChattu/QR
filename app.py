    ...
    if st.session_state.active_session is None:
        ...
        if mode == "Poll (MCQ)":
            question = st.text_input(
                "Poll question",
                placeholder="ex: Which platform do you use most?"
            )
            options_raw = st.text_input(
                "Options (comma-separated)",
                placeholder="ex: WhatsApp, Instagram, Facebook, Snapchat"
            )
            if st.button("🚀 Start Poll Session"):
                if not question.strip() or not options_raw.strip():
                    st.warning("Please fill in the question and options.")
                else:
            ...
            question = st.text_input(
                "Opinion question",
                placeholder="ex: What is your opinion about social media?"
            )

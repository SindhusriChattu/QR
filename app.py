...
from rag_engine import build_index, retrieve_relevant, generate_grounded_answer
...
def host_view():
    ...
    if st.session_state.active_session is None:
        ...
        if mode == "Poll (MCQ)":
            question = st.text_input("Poll question", "Which platform do you use most?")
            options_raw = st.text_input(
                "Options (comma-separated)",
                "WhatsApp, Instagram, Facebook, Snapchat"
            )
            if st.button("🚀 Start Poll Session"):
                options = [o.strip() for o in options_raw.split(",") if o.strip()]
                sid = str(uuid.uuid4())[:8]
                create_session(sid, "poll", question, options)
                st.session_state.active_session = sid
                st.rerun()

        else:
            question = st.text_input("Opinion question", "What is your opinion about social media?")
            if st.button("🚀 Start Opinion Session"):
                sid = str(uuid.uuid4())[:8]
                create_session(sid, "opinion", question)
                st.session_state.active_session = sid
                st.rerun()
